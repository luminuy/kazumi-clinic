...
const upsertSchema = z.object({
  ...
  detailEn: z.string().trim().max(200).nullish(),
  tagline: z.string().trim().max(120).nullish(),
  taglineEn: z.string().trim().max(120).nullish(),
  benefits: z.array(z.string().trim().min(1).max(300)).max(12).nullish(),
  benefitsEn: z.array(z.string().trim().min(1).max(300)).max(12).nullish(),
...
export async function POST(request: NextRequest) {
  ...
  const input: ProductInput = {
    ...
    benefitsEn: data.benefitsEn ?? null,
