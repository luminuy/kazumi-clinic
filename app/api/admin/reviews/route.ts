...
const upsertSchema = z
  .object({
    ...
    quoteEn: z.string().trim().max(600).nullish(),
    procedure: z.string().trim().max(120).nullish(),
    procedureEn: z.string().trim().max(120).nullish(),
...
export async function POST(request: NextRequest) {
  ...
  const input: ReviewInput = {
    ...
    procedureEn: data.procedureEn ?? null,
...
export async function DELETE(request: NextRequest) {
  ...
  try {
    await deleteReview(parsed.data.id, email);
    revalidatePath('/reviews');
    revalidatePath('/en/reviews');
...
export async function PUT(request: NextRequest) {
  ...
  try {
    await restoreReview(parsed.data.id, email);
    revalidatePath('/reviews');
    revalidatePath('/en/reviews');
    return NextResponse.json({ ok: true });
  } catch (error) {
