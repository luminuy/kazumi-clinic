...
- **แปลเว็บอังกฤษให้ครบและถูกหลักภาษา** (Claude วางแผน/ตรวจ · Codex แก้โค้ด · Gemini CLI แปล) — ทำเป็นเฟส:
  ...
  | 4b · **แปลเนื้อหาที่มีอยู่** (20 บทความ · 17 สินค้า · 2 โปรฯ) | 🔨 **บทความ 20/20 เสร็จแล้ว** (2026-08-02) — pipeline ใหม่ [scripts/i18n/blog-extract.sh](scripts/i18n/blog-extract.sh) → [translate-blog.sh](scripts/i18n/translate-blog.sh) (Gemini) → [blog-validate.mjs](scripts/i18n/blog-validate.mjs) → [blog-apply.mjs](scripts/i18n/blog-apply.mjs) (สร้าง SQL แล้วรันตรงเข้า production ด้วย `wrangler d1 execute --remote`) · validate มี warning 1 จุด (ชื่อหมอถูกปล่อยเป็นไทยตามกฎ ห้ามแปลชื่อเฉพาะ — ถูกต้องแล้ว) + false positive เรื่องตัวเลข 6 จุด (ตรวจมือแล้วว่าเป็นแค่สำนวน "1" → "a/first/single" ไม่ใช่ตัวเลขคลาดเคลื่อน) ยืนยันบน production จริงแล้ว (`/en/blog/article-11-skin-booster`, `/en/blog/article-1-filler`) · **17 สินค้า + 2 โปรฯ ยังไม่ทำ** — ใช้ pipeline เดียวกันได้ แค่เปลี่ยน query/ตาราง |
