...
- `gh pr checks <num>` — **repo นี้มี CI แล้วตั้งแต่ 2026-07-22** ([.github/workflows/ci.yml](.github/workflows/ci.yml): lint + typecheck + test + build บนทุก PR) → ต้องรอให้ check **SUCCESS ก่อน merge เสมอ** · ⚠️ **ยังบังคับด้วย branch protection ไม่ได้** เพราะ private repo ต้อง GitHub Pro หรือทำ repo เป็น public (ทั้ง classic protection + rulesets ตอบ 403 · ตรวจ 2026-07-22) — enforcement จึงเป็น *soft*: ยึดกฎ "ผ่าน PR + CI เขียวเสมอ" + CI รันบน `push: main` ด้วย ถ้าใครดันของพังเข้า main ตรง ๆ CI จะจับแล้ว GitHub เมลเตือน (detection ไม่ใช่ prevention) · มี **local pre-push hook** ([.githooks/pre-push](.githooks/pre-push)) กัน `git push` ตรงเข้า main (เปิดใช้อัตโนมัติตอน `pnpm install` ผ่าน `core.hooksPath`, หรือ `pnpm setup:hooks`) — bypass ได้ด้วย `--no-verify` และทำงานเฉพาะเครื่องที่ติดตั้ง hook
