...
- `gh pr checks <num>` — **repo นี้มี CI แล้วตั้งแต่ 2026-07-22** ([.github/workflows/ci.yml](.github/workflows/ci.yml): lint + typecheck + test + build บนทุก PR) → ต้องรอให้ check **SUCCESS ก่อน merge เสมอ** · ⚠️ **ยังบังคับด้วย branch protection ไม่ได้** เพราะ private repo ต้อง GitHub Pro หรือทำ repo เป็น public (ทั้ง classic protection + rulesets ตอบ 403 · ตรวจ 2026-07-22) — enforcement จึงเป็น *soft*: ยึดกฎ "ผ่าน PR + CI เขียวเสมอ" + CI รันบน `push: main` ด้วย ถ้าใครดันของพังเข้า main ตรง ๆ CI จะจับแล้ว GitHub เมลเตือน (detection ไม่ใช่ prevention) · ตัวช่วยฝั่งเครื่องคือ **local hook 2 ตัว** (เปิดใช้อัตโนมัติตอน `pnpm install` ผ่าน `core.hooksPath`, หรือ `pnpm setup:hooks`):
...
- 2026-07-23 — **CI แดงค้างบน main เพราะงานถูก commit ลง main ตรง ๆ แล้ว push ข้าม hook — pre-push hook ตัวเดียวกันไม่ทัน** · `357b8d5` ทำ main แดงที่ lint (`lib/session.ts` ใช้ `payload as any` ยัดเข้า `JWTPayload` ของ jose → ชน `no-explicit-any` ซึ่งเป็น *error* ไม่ใช่ warning) · ไล่ `git reflog show main` เจอว่า commit 4 ตัว (08:32–09:08) ถูก **commit ลง `main` โดยตรงในโฟลเดอร์หลัก** แล้ว ff-merge `feature/login-popup` เข้า main ตามด้วย rebase — พอถึงขั้น push งานอยู่บน main หมดแล้ว ทางเดียวที่เหลือคือ `--no-verify` (หรือ tool ที่ไม่ยิง hook) · ทดสอบแล้ว hook เดิม *ทำงานถูกต้อง* — มันแค่อยู่ปลายทางเกินไป และไม่เคยเช็ค lint เลย
  ...
  → ยืนยันซ้ำ 2026-07-23: branch protection/rulesets ยัง **403 "Upgrade to GitHub Pro or make this repository public"** — ยังไม่มี prevention ฝั่ง server
