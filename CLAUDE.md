...
- 🔒 **ความปลอดภัยมาก่อน — CD ทำให้ "merge เข้า main = เผยแพร่สู่สาธารณะทันที" ไม่มีด่านมนุษย์คั่นอีกแล้ว** · ด่านตรวจความปลอดภัยจึงต้องขยับมา **ก่อน merge** ไม่ใช่ก่อน deploy · ก่อนกด merge งานที่แตะ Worker/DB/auth/เนื้อหา ต้องผ่านทุกข้อ:
  - **ความลับ**: ห้ามมี secret/token/คีย์ hardcode หรือ fallback ด้วย `||` (ดู §0.5 · repo เป็น public → หลุดถาวร) · ⚠️ ปมค้างที่รู้ตัวแล้ว: `lib/session.ts` มี `SESSION_SECRET || 'default-...'` และ production **ยังไม่เคยตั้ง `SESSION_SECRET`** — **ห้าม merge Phase 3/4 (member auth/checkout ที่เรียก `getSession()`) จนกว่าจะ `wrangler secret put SESSION_SECRET` จริงก่อน** ไม่งั้น CD จะดันของที่ปลอม session ได้ขึ้น public เอง
