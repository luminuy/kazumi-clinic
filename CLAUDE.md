...
| ย้าย asset ข้าม origin / ไล่แก้ LCP ให้คะแนน Lighthouse ขึ้น | **2026-08-04 (ลดจำนวน origin ทำให้ FCP+LCP แย่ลง ต้อง revert · วัดต้องเก็บ ≥3 รอบดู median)** |
...
- 2026-08-04 — **ย้ายรูปมาเสิร์ฟผ่าน origin ตัวเองเพื่อตัด DNS+TCP+TLS ที่ขวางหน้ารูป LCP แล้ว "ทั้ง FCP และ LCP แย่ลง" ต้อง revert ทั้งดุ้น** (PR #311 → revert #312) · เหตุผลที่ตั้งไว้ฟังขึ้นและมีข้อมูลรองรับ: `Load Delay` เป็นเฟสที่ใหญ่ที่สุดของ LCP และ**ไม่ขยับเลย**ตอนบล็อก RSC prefetch (93KB) หรือบล็อกฟอนต์ทั้งหมด (101KB) → สรุปว่าไม่ใช่แบนด์วิดท์ แต่เป็นค่าเปิด connection ไป origin ที่สอง · แต่พอทำจริงแล้ววัด 6 รอบ: FCP median 2,622 → **3,166ms** · LCP median 4,564 → **5,124ms** · สาเหตุที่พลาด: same-origin ตัด handshake ได้จริง แต่ `<link rel="preload" as="image" fetchpriority="high">` ของรูป hero อยู่ที่ byte 280 ของ HTML ซึ่ง**มาก่อน** `<link stylesheet>` ที่ byte 1351 — พอย้ายมา origin เดียวกัน มันเลยไปแย่ง connection เดียวกับ CSS ที่บล็อกการ render · สิ่งที่ประหยัดได้ (4 RTT) น้อยกว่าสิ่งที่เสีย (CSS มาช้าลง → FCP ช้าลง → LCP ช้าตาม)
  → **กฎ: "ลดจำนวน origin" ไม่ใช่ของฟรี** — cross-origin เสีย handshake แต่ได้ connection แยกที่ไม่แย่งกับ critical CSS · ก่อนย้าย asset ข้าม origin ต้องดูก่อนว่า resource hint ของมันอยู่**ก่อนหรือหลัง** stylesheet ที่บล็อก render ใน `<head>` ถ้าอยู่ก่อน การรวม origin จะทำให้ FCP แย่ลงเสมอ
  → **กฎ: เหตุผลที่ฟังขึ้น + ข้อมูลที่ตัดตัวแปรอื่นออกแล้ว ยังไม่พอที่จะเชื่อว่า "แก้แล้วจะดีขึ้น"** · การตัดตัวแปรออก (บล็อก prefetch/ฟอนต์) พิสูจน์ได้แค่ว่า *อะไรไม่ใช่สาเหตุ* ไม่ได้พิสูจน์ว่า *อะไรคือทางแก้* · ต้องวัดหลัง deploy จริงเสมอ แล้ว**ยอม revert ถ้าตัวเลขไม่ขึ้น** ไม่ใช่หาเหตุผลให้มันอยู่ต่อ
  → **กฎ: วัด perf บนเครื่อง dev นี้ต้องเก็บอย่างน้อย 3 รอบแล้วดู median เสมอ** · รันเดี่ยว ๆ ของ URL เดิมให้ TBT ตั้งแต่ 70ms ถึง 2,600ms (Chrome ของ user เปิดค้างแย่ง CPU) — ตัวเลขรอบเดียวตัดสินใจอะไรไม่ได้เลย · metric ที่นิ่งพอใช้ตัดสินคือ FCP/LCP median, ส่วน TBT/SI ให้ถือว่าเป็น noise จนกว่าจะวัดที่อื่น
